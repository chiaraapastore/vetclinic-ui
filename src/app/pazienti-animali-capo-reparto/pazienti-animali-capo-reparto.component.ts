...
@Component({
  selector: 'app-pazienti-animali-capo-reparto',
  standalone: false,
  templateUrl: './pazienti-animali-capo-reparto.component.html',
  styleUrl: './pazienti-animali-capo-reparto.component.css'
})
export class PazientiAnimaliCapoRepartoComponent implements OnInit {
  ...
  loadAnimali(): void {
    this.capoRepartoService.getPazientiDelReparto().subscribe({
      next: (data) => {
        this.animali = data;
      },
      error: (err: any) => {
  ...
  loadAnimalHistory(animaleId: number, nomeAnimale: string): void {
    ...
    this.capoRepartoService.getAnimalFullHistory(animaleId).subscribe({
  ...
  addNewEvent(): void {
    ...
    this.capoRepartoService.addEventToAnimal(this.selectedAnimalId, {
